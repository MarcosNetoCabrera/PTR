...
Matrix matrix_inverse(Matrix matrix){
    ...
    double tmp=0;
    for(int k = 0; k < tam;k++){
        tmp = VALUES(matrix,k,k);
        for(int j = 0;j < tam;j++){
            VALUES(matrix,k,j)/=tmp;
            VALUES(identidade,k,j)/=tmp;
        }
        for(int i=0;i<tam;i++){

            tmp = VALUES(matrix,i,k);

            for(int j=0; j < tam;j++){
                if(i==k) break;

                VALUES(matrix,i,j) -= VALUES(matrix,k,j) * tmp;
                VALUES(identidade,i,j) -= VALUES(identidade,k,j) * tmp;
            }
        }
    }
    return identidade;
